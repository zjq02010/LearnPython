...
from pyloco.util import parse_param_option, pyloco_formatter, system
...
"""
    ...
    def _setup(self, taskpath):
        ...
        self._command_handlers = {
                ...
                "pyloco": self.run_pyloco_command,
            }
    ...
    def run(self, argv, subargv=None, forward=None):
        ...
        self._parser.prog = "pyloco " + prog[-20:]
    ...
    def run_section(self, body, *vargs, **kwargs):
        ...
        for b in body:
            ...
            l3 = pyloco_formatter.vformat(l2, [], self._env)
            #l4 = l3.replace("}}", "}").replace("{{", "{")
    ...
    def run_pyloco_command(self, cmd, *vargs, **kwargs):

        from pyloco import perform
        ...
        if cmd.startswith("-"):
            if cmd.startswith("-- "):
                return perform("", "", cmd[3:])
            else:
                idx = cmd.find("-- ")
                if idx > 0:
                    return perform("", cmd[:idx], cmd[idx+3:])
                else:
                    return perform("", cmd)
        else:
            return perform("", "", cmd)
